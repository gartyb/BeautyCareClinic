...
- 🟡 **עודכן 2026-07-26, אומת ישירות מול השרת החי** — התברר ש-`888/tcp` **אינו** פאנל הניהול של aaPanel (הנחה מוטעית שהייתה בממצא המקורי) — הפורט הרשמי של הפאנל אושר כ-`25664` (ראה למטה, `data/port.pl`). על `888/tcp` כן מאזין תהליך nginx נפרד, אך תפקידו בפועל לא אושש עדיין. **תיקון:** לזהות מה בדיוק רץ על 888 לפני החלטה על סגירה/הגבלה.
- 🟢 **אומת + נפתר סופית (2026-07-26)** — פורט `25664/tcp` הוא **אכן** פורט הניהול הרשמי והמכוון של aaPanel (`data/port.pl` = `25664`), לא תהליך-עזר לא-מזוהה כפי שנחשד קודם. הוא מוגן בשתי שכבות מעבר לשם-משתמש/סיסמה: (א) "נתיב כניסה מאובטח" סודי (`data/admin_path.pl`), ו-(ב) שער טביעת-אצבע ללקוח מוכר (`check_client_info()` ב-`class/public/common.py:9091`) שמחזיר דף 404 מזויף, זהה בייט-לבייט לדף השגיאה הסטנדרטי של nginx, לכל שילוב IP+User-Agent שלא התחבר בהצלחה ב-30 הימים האחרונים — שוחזר בהצלחה מול לקוח לא-מוכר בבדיקה החוזרת. **חומרה מעודכנת: נמוכה** (במקום 🟡 בהמתנה לזיהוי) — ההשערה המקורית שהפורט חושף בטעות את אפליקציית Beauty Clinic נבדקה ונשללה. **המלצה:** לתעד את הפורט במסמכי הארכיטקטורה; שקילת הגבלת כלל ה-ufw ל-IP ניהול ידוע כהגנה נוספת (defense-in-depth), מאחר ששער טביעת-האצבע הוא בקרה יחידה ולא גבול-רשת.
...
- ❔? `888/tcp` — תהליך nginx נפרד מאזין כאן; תפקידו בפועל לא אושש (ראה עדכון 2026-07-26 למעלה — זה **אינו** פאנל aaPanel, כפי שהונח בטעות קודם; הפורט האמיתי הוא 25664, שנפתר סופית).
...
- פריטים שדרשו אימות נוסף: זהות השירות על `888/tcp` (25664 נפתר סופית ב-2026-07-26); האם `docker`-group permission חסרה למשתמש `runner` נבדקה בעקיפין (`sudo docker` עבד, `docker` ישיר נכשל) — לא נבדק אם זו כוונה או פער.
